...
COMPONENT_LABELS = {
    "Q_fit":          "ODE Fit\n(Q_fit)",
    "Q_backfire":     "Backfire\nAccuracy\n(Q_back)",
    "Q_substitution": "T–C Sub-\nstitution\n(Q_sub)",
    "Q_user":         "User\nConsistency\n(Q_user)",
    "Q_regime":       "Regime\nMargin\n(Q_reg)",
    "Q_spillover":    "Spillover\nPersistence\n(Q_spill)",
}
...
def plot_tc_substitution(daily_metrics_path: Path, plot_dir: Path):
    """
...
def make_table1(mci: pd.DataFrame, tables_dir: Path):
    ...
    Columns: Subreddit | Type | Regime | Q_fit | Q_back | Q_sub | Q_user | Q_reg | Q_spill | MCI | N_components
    """
    df = mci.copy().sort_values("MCI", ascending=False)
    df["Type"] = df["is_banned"].map({True: "Banned", False: "Control"})

    keep = ["subreddit", "Type", "regime",
            "Q_fit", "Q_backfire", "Q_substitution",
            "Q_user", "Q_regime", "Q_spillover",
            "MCI", "n_components", "g_star"]
    df = df[[c for c in keep if c in df.columns]]

    # Round floats
    float_cols = ["Q_fit", "Q_backfire", "Q_substitution",
                  "Q_user", "Q_regime", "Q_spillover", "MCI", "g_star"]
    for c in float_cols:
        if c in df.columns:
            df[c] = df[c].round(4)
...
def make_table3(scalars: dict, mci: pd.DataFrame, tables_dir: Path):
    ...
    rows = [
        ...
        # Q3 — T–C substitution
        {
            "Test":        "T–C substitution partial regression (Q_substitution)",
            "Statistic":   f"t = {q_sub.get('beta1_t', np.nan):.4f}",
            "p_value":     q_sub.get("beta1_p", np.nan),
            "Effect_size": f"partial r = {q_sub.get('partial_r_TC', np.nan):.4f}",
            "Null_H0":     "β₁ = 0 (no substitution)",
            "Direction":   "β₁ < 0",
            "Confirmed":   "YES" if q_sub.get("beta1_TC", 0) < 0
                           and q_sub.get("beta1_p", 1) < 0.05 else "NO",
            "Reference":   "Tyler (1990); Levi (1988)",
        },
        # Q4 — ANOVA
        {
        ...
        # Q6 — spillover Mann-Whitney
        {
            "Test":        "Spillover: migrant INITIATOR > native removal rate (Mann-Whitney)",
            "Statistic":   f"U = {q_spill.get('mw_U', np.nan):.0f}",
            "p_value":     q_spill.get("mw_p", np.nan),
            "Effect_size": f"Cohen's d = {q_spill.get('cohens_d', np.nan):.4f}",
            "Null_H0":     "Migrant removal rate ≤ native removal rate",
            "Direction":   "Migrant > native",
            "Confirmed":   "YES" if q_spill.get("mw_p", 1) < 0.05 else "NO",
            "Reference":   "Guerette & Bowers (2009); Chandrasekharan et al. (2017)",
        },
        # Q6 — dose-response
        {
            "Test":        "Dose-response: theta_proxy → post-ban removal rate (OLS)",
            "Statistic":   f"β₁ = {q_spill.get('dose_beta_theta', np.nan):.6f}",
            "p_value":     q_spill.get("dose_beta_p", np.nan),
            "Effect_size": f"R² = {q_spill.get('dose_r2', np.nan):.4f}",
            "Null_H0":     "β₁ = 0 (theta_proxy not predictive of post-ban behavior)",
            "Direction":   "β₁ > 0",
            "Confirmed":   "YES" if q_spill.get("dose_beta_theta", 0) > 0
                           and q_spill.get("dose_beta_p", 1) < 0.05 else "NO",
            "Reference":   "Granovetter (1978) — intrinsic motivation persistence",
        },
        # Sensitivity
        {
...
def print_summary(t1: pd.DataFrame, t2: pd.DataFrame, t3: pd.DataFrame):
    ...
    pred_map = {
        "P1 (Backfire g*)":      "Backfire prediction accuracy",
        "P2 (T–C Substitution)": "T–C substitution partial regression",
        "P3 (User Types)":       "User behavioral consistency ANOVA",
        "P4 (Spillover)":        "Spillover: migrant INITIATOR",
    }
